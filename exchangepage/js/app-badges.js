...
async function j(url) {
  try {
    ...
    if (!res.ok) {
      console.warn('[badge] HTTP', res.status, url);
      return null;
    }
    return await res.json();
  } catch (e) {
    console.warn('[badge] fetch', url, e);
    return null;
  }
}
const arrLen = p => Array.isArray(p) ? p.length
  : (p && Array.isArray(p.items)) ? p.items.length
  : (p && Array.isArray(p.data)) ? p.data.length
  : null;

/* ---------- Favorites ---------- */
async function refreshFavBadge() {
  const el = getEl('favBadge');
  if (!el) return;
...
async function refreshReqBadge() {
  ...
  if (!el) return;
  try {
    const r = await fetch('../api/requests/list_incoming.php?status=pending&limit=1', {
...
async function refreshChatBadge() {
  ...
  if (!el) return;
  const data = await fetch('../api/chat/unread_count.php', {
    credentials: 'include', cache: 'no-store'
...
async function refreshAll() {
  await Promise.all([refreshFavBadge(), refreshReqBadge(), refreshChatBadge()]);
}
