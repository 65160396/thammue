<?php
// /page/backend/ex_decline_request.php
...
try {
  ...
  $st = $m->prepare("UPDATE ".T_REQUESTS." SET status='declined', updated_at=NOW() WHERE id=?");
  $st->bind_param("i", $rid);
  $st->execute();

  $typ='request_declined'; $title='คำขอแลกถูกปฏิเสธ'; $body='ผู้ขายปฏิเสธคำขอแลกของคุณ';
