...
if ((int)$r['requester_user_id'] !== $uid) jerr('forbidden', 403);
if ($r['status'] !== 'pending') jerr('invalid_status', 400);

$st = $mysqli->prepare("UPDATE ex_requests SET status='cancelled', updated_at=NOW() WHERE id=?");
$st->bind_param("i", $rid);
$st->execute();

$user_id = (int)$r['owner_user_id'];
$typ='status'; $title='ผู้ขอยกเลิกคำขอ'; $body='ผู้ใช้ยกเลิกคำขอแลกเปลี่ยนแล้ว';
